...
ReverseProxy.prototype.setupHttpProxy = function (proxy, websocketsUpgrade, log, opts) {
  ...
  var server = this.server = http.createServer(function (req, res) {
    ...
    var target = _this._getTarget(src, req);
    if (target) {
      if (shouldRedirectToHttps(_this.certs, src, target)) {
...
ReverseProxy.prototype.setupLetsencrypt = function (log, opts) {
  ...
  var challengeResolver = function (host, url) {
    if (/^\/.well-known\/acme-challenge/.test(url)) {
      return 'http://127.0.0.1:' + letsencryptPort + '/' + host;
    }
  };
...
ReverseProxy.prototype.setupHttpsProxy = function (proxy, websocketsUpgrade, log, opts) {
  ...
  var leSni;
  if (this.opts.letsencrypt) {
    leSni = require('le-sni-auto').create({
      renewWithin: 10 * 24 * 60 * 60 * 1000
    });
  }
  // if (this.opts.letsencrypt) {
  //   var le = letsencrypt.create(this.opts.letsencrypt.production, this.opts.letsencrypt);
  //   leSni = le.sni;
  // }
  ...
  var ssl = {
    ...
    //
    key: getCertData(opts.ssl.key) || undefined,
    cert: getCertData(opts.ssl.cert) || undefined
  };
  ...
  ssl = _.defaults(ssl, require('localhost.daplie.com-certificates').merge({}));
...
ReverseProxy.prototype.updateCertificates = function (domain, email, production) {
  var _this = this;

  function renewCertificate() {
    _this.log & _this.log.info('Renewing letscrypt certificates for %s', domain);
    _this.updateCertificates(domain, email, production);
  }

  return letsencrypt.getCertificates(domain, email, production, _this.opts.letsencrypt, this.log).then(function (certs) {
    if (certs) {
      var opts = {
        key: certs.privkey,
      ...
      //
      var timeBeforeExpiration = _this.opts.letsencrypt.expireBefore || ONE_MONTH;
